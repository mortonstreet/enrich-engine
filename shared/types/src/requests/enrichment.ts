...
export const EmailEnrichListRequestSchema = z.object({
  organizationId: z.string().min(1),
  listId: z.string().uuid(),
  useProspeo: z.boolean().optional().default(false),
  forceRefresh: z.boolean().optional().default(false),
  verifyGeneratedEmails: z.boolean().optional().default(false),
});
export type EmailEnrichListRequest = z.infer<typeof EmailEnrichListRequestSchema>;
...
export interface EmailEnrichListResponse {
  totalLeads: number;
  emailsGuessed: number;
  emailsFoundByProspeo: number;
  emailsFoundByLinkedIn: number;
  emailsFromPatternLearning: number;
  ...
  emailVerificationJobsQueued: number;
  skipped: number;
  creditsUsed: number;
