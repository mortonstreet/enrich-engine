...
> = async (req, res) => {
  ...
  try {
    ...
    // Return empty array if Twilio is not configured or credentials are invalid
    res.json({ data: [] })
  }
}
...
> = async (req, res) => {
  ...
  try {
    ...
    // Return empty array if Twilio is not configured
    res.json({ data: [] })
  }
}
...
> = async (req, res) => {
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('TWILIO_API_KEY')) {
        return res.status(500).json({
          error:
            'Twilio API key not configured. Please contact your administrator.',
        })
      }
      if (
...
export const initiateCall: AuthRequestHandler<InitiateCallRequest> = async (
  ...
  if (!twilioConfig) {
    return res
      .status(400)
      .json({ error: 'Twilio not configured for this organization' })
  }
...
export const getCallRecording: AuthRequestHandler<GetCallRequest> = async (
  ...
  // Get Twilio config to authenticate the recording request
  const twilioConfig = await dialerService.resolveOrgTwilioConfig(
    organizationId,
    req.user.id,
  )
  ...
  try {
    // Proxy the recording from Twilio with proper authentication
    // The auth token is stored encrypted, so we need to decrypt it
    const decryptedAuthToken = decryptAuthToken(twilioConfig.authTokenEncrypted)
    const authString = Buffer.from(
      `${twilioConfig.accountSid}:${decryptedAuthToken}`,
    ).toString('base64')
