...
// === Research Provider Connection ===

// Test research provider connection (before saving)
router.post(
  '/connection/test',
  withBetterAuth,
  async (req, res: Response, next: NextFunction) => {
    try {
      const { apiKey, provider, baseUrl } =
        TestFirecrawlConnectionRequestSchema.parse(req.body)

      if (provider === 'crawl4ai') {
        const result = await crawl4aiClient.testConnection(apiKey, { baseUrl })
