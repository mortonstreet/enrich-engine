...
import { EmailDomainProfileProcessor } from '@/queues/email-domain-profile.worker'
import { EmailCandidateVerifyProcessor } from '@/queues/email-candidate-verify.worker'
...
export const startWorker = async () => {
  ...
  const emailDomainProfileProcessor = new EmailDomainProfileProcessor()
  const emailCandidateVerifyProcessor = new EmailCandidateVerifyProcessor()
  ...
  const shutdown = async () => {
    ...
    await emailDomainProfileProcessor.close()
    await emailCandidateVerifyProcessor.close()
