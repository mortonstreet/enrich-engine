...
// Detect phone type using Twilio Lookup API
export const detectPhoneType = async (
  ...
  try {
    ...
    const lineType = (lookup as any).lineTypeIntelligence?.type || 'unknown'
    ...
    return {
      ...
      phoneType,
      carrier: (lookup as any).lineTypeIntelligence?.carrier_name || null,
      mobileCountryCode:
        (lookup as any).lineTypeIntelligence?.mobile_country_code || null,
      mobileNetworkCode:
        (lookup as any).lineTypeIntelligence?.mobile_network_code || null,
    }
  } catch (error) {
    return {
      phoneNumber,
