...
export const dialNextBatch = async (
  ...
  console.log('[ParallelDialer] Getting Twilio client for org:', organizationId)
  const twilio = await twilioClient.getClientForOrganization(organizationId)
  console.log('[ParallelDialer] Got Twilio client, initiating calls')
  ...
  const dialPromises = attempts.map(async (attempt, index) => {
    try {
      ...
      const webhookUrl = `${config.backendUrl}/api/webhooks/twilio/parallel-dial-answered?attemptId=${attempt.id}&sessionId=${sessionId}`
      ...
      const call = await twilio.calls.create({
        ...
        from: fromNumber,
        url: webhookUrl,
        statusCallback: `${config.backendUrl}/api/webhooks/twilio/parallel-dial-status?attemptId=${attempt.id}`,
        ...
        asyncAmd: 'true',
        asyncAmdStatusCallback: `${config.backendUrl}/api/webhooks/twilio/parallel-dial-amd?attemptId=${attempt.id}`,
      })
...
export const handleCallAnswered = async (
  ...
  // Idempotency: Twilio may retry answered webhooks.
  ...
  for (const otherAttempt of activeAttempts) {
    if (otherAttempt.id !== attemptId && otherAttempt.callSid) {
      try {
        ...
        await twilio.calls(otherAttempt.callSid).update({
          url: `${config.backendUrl}/api/webhooks/twilio/parallel-dial-abandoned`,
          method: 'POST',
        })
      } catch (e) {
...
export const endSession = async (
  ...
  for (const attempt of activeAttempts) {
    if (attempt.callSid) {
      try {
        await twilio.calls(attempt.callSid).update({ status: 'completed' })
      } catch (e) {
