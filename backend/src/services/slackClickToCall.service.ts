...
export async function initiateCallFromSlack(
  ...
  try {
    ...
    // When answered, TwiML will put them in a conference and dial the lead
    await client.calls.create({
      to: repPhone,
      from: fromNumber,
      url: `${config.backendUrl}/api/webhooks/twilio/slack-call-rep-answered?callId=${callRecord.id}&leadPhone=${encodeURIComponent(leadPhone)}&fromNumber=${encodeURIComponent(fromNumber)}&conferenceId=${encodeURIComponent(conferenceId)}`,
      statusCallback: `${config.backendUrl}/api/webhooks/twilio/slack-call-status?callId=${callRecord.id}`,
...
export async function handleRepAnswered(params: {
  ...
  if (!twilioConfig) {
    throw new Error(`Twilio config not found for call: ${callId}`)
  }
  ...
  setImmediate(async () => {
    try {
      ...
      await client.calls.create({
        to: leadPhone,
        from: fromNumber,
        url: `${config.backendUrl}/api/webhooks/twilio/slack-call-bridge?callId=${callId}&conferenceId=${encodeURIComponent(conferenceId)}`,
        statusCallback: `${config.backendUrl}/api/webhooks/twilio/slack-call-lead-status?callId=${callId}`,
        statusCallbackEvent: ['initiated', 'ringing', 'answered', 'completed'],
      })
  ...
  // Return TwiML to put rep in conference
  return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Say>Connecting you now. Please wait.</Say>
  <Dial>
    <Conference startConferenceOnEnter="true" endConferenceOnExit="true" beep="false" record="record-from-start" recordingStatusCallback="${config.backendUrl}/api/webhooks/twilio/recording?callId=${callId}">${conferenceId}</Conference>
  </Dial>
</Response>`
}
...
export async function handleCallStatusUpdate(params: {
  ...
  // Map Twilio status to user-friendly message
