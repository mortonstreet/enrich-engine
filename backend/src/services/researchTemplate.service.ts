...
export const SYSTEM_TEMPLATES = [
  {
    name: 'Studio Qualification (Crawl4AI + Qwen)',
    description:
      'Qualify animation/production studios and verify animator hiring in the last 30 days',
    prompt: `# CONTEXT
You are a research agent qualifying businesses for outreach by an animation, VFX, and 3D studio.
Use the provided company domain to determine whether the business is an animation studio or production studio and whether it is currently hiring animators with a posting published within the last 30 days.

# OBJECTIVE
Verify if the business associated with the domain is an animation studio or production studio and confirm an animator-related job posting from the last 30 days.
Return structured JSON only.

# INSTRUCTIONS
1. Start from the exact domain. Prefer the company's public website first.
2. Determine company type using evidence from About/Services/Work pages:
   - animationStudio, productionStudio, or notStudio.
3. Find animator-related jobs:
   - Prefer company careers pages.
   - If missing, use reputable external listings that clearly match the same company/domain.
4. Date validation:
   - A posting qualifies only if it was published within the last 30 days.
   - Use posted/published date text exactly as shown when available.
5. Evidence:
   - Capture jobTitle, jobUrl, postingDateText, and a short evidenceSnippet.
   - If multiple valid jobs exist, use the most recent.
6. Constraints:
   - Do not fabricate.
   - Set unknown fields to null and explain in reason.
   - Return one JSON object only.`,
    targetUrls: [
      '{company_website}',
      '{company_website}/about',
      '{company_website}/services',
      '{company_website}/work',
      '{company_website}/portfolio',
      '{company_website}/careers',
      '{company_website}/jobs',
      '{linkedin_url}',
    ],
    extractionSchema: {
      domain: {
        description: 'Company domain being researched',
        type: 'string',
        required: true,
      },
      companyType: {
        description:
          'Company type: animationStudio, productionStudio, or notStudio',
        type: 'string',
        required: true,
      },
      qualifies: {
        description:
          'True only if company is animation/production studio and has animator-related posting in last 30 days',
        type: 'boolean',
        required: true,
      },
      jobTitle: {
        description: 'Most relevant animator-related job title',
        type: 'string',
      },
      jobUrl: {
        description: 'URL for the most relevant qualifying job posting',
        type: 'string',
      },
      postingDateText: {
        description: 'Visible posting date text exactly as shown',
        type: 'string',
      },
      evidenceSnippet: {
        description:
          'Short quote/snippet proving studio type or recent animator hiring',
        type: 'string',
      },
      reason: {
        description:
          'Clear reason for qualification decision with evidence and recency validation',
        type: 'string',
        required: true,
      },
      response: {
        description: 'Concise human-readable qualification response',
        type: 'string',
      },
      reasoning: {
        description: 'Detailed reasoning path behind the decision',
        type: 'string',
      },
      confidence: {
      ...
      stepsTaken: {
        description: 'Visited pages/actions taken during research',
        type: 'array',
      },
    },
    fieldMappings: {
      ...
      qualifies: 'studio_job_qualifies',
      jobTitle: 'job_title',
      jobUrl: 'job_url',
      postingDateText: 'posting_date_text',
      ...
      stepsTaken: 'research_steps_taken',
    },
  },
  {
