...
export const exportListLeads = async (
  ...
  const customFieldMaps = leads.map((lead) => {
    if (!lead.customFields) return {} as Record<string, string>
    if (typeof lead.customFields === 'string') {
      try {
        ...
        // Ignore invalid JSON custom fields
      }
    }
    if (typeof lead.customFields === 'object') {
      return lead.customFields as Record<string, string>
    }
    return {} as Record<string, string>
  })

  const customFieldHeaders = Array.from(
    new Set(
      customFieldMaps.flatMap((fields) =>
        Object.keys(fields).filter((key) => key && !headers.includes(key)),
      ),
    ),
  )
  ...
  const rows = leads.map((lead, index) =>
    [
      ...
      ...customFieldHeaders.map((header) => {
        const value = customFieldMaps[index]?.[header] ?? ''
