...
/**
 * Get Twilio credentials for the organization
 */
async function getTwilioCredentials(organizationId: string) {
  // First try to get from TwilioConfig
  ...
  if (!twilioConfig || twilioConfig.phoneNumbers.length === 0) {
    throw new Error('No Twilio credentials configured')
  }

  return {
    accountSid: twilioConfig.accountSid,
    authToken: decrypt(twilioConfig.authTokenEncrypted),
    fromNumber: twilioConfig.phoneNumbers[0],
  }
}
...
export async function processEnrollment(
  ...
  try {
    ...
    const creds = await getTwilioCredentials(enrollment.organizationId)
