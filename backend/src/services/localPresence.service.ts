...
export const syncPhonePoolFromTwilio = async (
  ...
  const twilioNumbers = await twilio.incomingPhoneNumbers.list()
  ...
  for (const number of twilioNumbers) {
    ...
    const areaCode = extractAreaCode(number.phoneNumber)
    ...
    if (existing) {
      ...
      await db
        ...
        .set({
          ...
          capabilities: buildCapabilities(number.capabilities),
          updatedAt: new Date(),
        })
      ...
      await db
        ...
        .values({
          ...
          region: (number as any).region || null,
          country: 'US',
          twilioSid: number.sid,
          capabilities: buildCapabilities(number.capabilities),
          isActive: true,
  ...
  // Mark numbers that no longer exist in Twilio as inactive
  const twilioPhoneNumbers = new Set(twilioNumbers.map((n) => n.phoneNumber))
  ...
  for (const existing of existingNumbers) {
    if (!twilioPhoneNumbers.has(existing.phoneNumber)) {
      await db
        .updateTable('phone_number_pool')
