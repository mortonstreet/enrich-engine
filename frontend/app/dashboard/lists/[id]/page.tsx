...
import { useEmailEnrichList, useEnrichmentVendors } from "@/hooks/api/useEnrichment";
...
export default function ListDetailPage() {
  ...
  const [isEmailEnrichOpen, setIsEmailEnrichOpen] = useState(false);
  const [missingEmailCount, setMissingEmailCount] = useState(0);
  const [isCalculatingMissing, setIsCalculatingMissing] = useState(false);
  const [useProspeo, setUseProspeo] = useState(false);
  ...
  const { data: enrichmentVendors } = useEnrichmentVendors();
  ...
  const emailEnrichMutation = useEmailEnrichList();

  const leads = leadsData?.data ?? [];
  const pagination = leadsData?.pagination;
  const campaigns = campaignsData?.data ?? [];
  const pipelineStages = stagesData?.data ?? [];
  const hasProspeoConnection =
    enrichmentVendors?.data?.some(
      (vendor) => vendor.provider === "prospeo" && vendor.isActive
    ) ?? false;
  ...
  const countMissingEmailsInList = async (): Promise<number> => {
    if (!orgId) return 0;
    ...
    while (true) {
      ...
      }>(`${ENDPOINTS.LISTS.LEADS(listId)}?${searchParams.toString()}`);

      missingEmails += response.data.filter((lead) => !lead.email?.trim()).length;
  ...
  const openEmailEnrichDialog = async () => {
    setIsCalculatingMissing(true);
    try {
      const missing = await countMissingEmailsInList();
      setMissingEmailCount(missing);
      setUseProspeo(hasProspeoConnection);
      setIsEmailEnrichOpen(true);
    } catch {
      toast.error("Failed to load email enrichment stats");
    } finally {
      setIsCalculatingMissing(false);
    }
  };

  const handleEmailEnrich = async () => {
    try {
      const result = await emailEnrichMutation.mutateAsync({
        listId,
        useProspeo: hasProspeoConnection ? useProspeo : false,
      });

      setIsEmailEnrichOpen(false);
      await refetch();

      toast.success(
        `Email enrichment complete. Guessed: ${result.emailsGuessed}, Prospeo: ${result.emailsFoundByProspeo}, Skipped: ${result.skipped}.`
      );
    } catch {
      toast.error("Failed to enrich emails for this list");
    }
  };
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={openEmailEnrichDialog}
            disabled={
              list.leadCount === 0 ||
              isCalculatingMissing ||
              emailEnrichMutation.isPending
            }
          >
            {isCalculatingMissing || emailEnrichMutation.isPending ? (
              <Loader2 className="w-4 h-4 mr-2 animate-spin" />
            ) : (
      ...
      {/* Email Enrich Dialog */}
      <Dialog open={isEmailEnrichOpen} onOpenChange={setIsEmailEnrichOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Enrich Emails</DialogTitle>
            <DialogDescription>
              {missingEmailCount > 0
                ? `${missingEmailCount.toLocaleString()} lead${missingEmailCount === 1 ? "" : "s"} in this list currently have no email.`
                : "No leads are currently missing an email, but you can still run enrichment."}
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 pt-2">
            {hasProspeoConnection ? (
              <div className="flex items-center justify-between rounded-lg border p-3">
                <div>
                  <p className="text-sm font-medium">Use Prospeo fallback</p>
                  <p className="text-xs text-muted-foreground">
                    Use Prospeo for leads not resolved by pattern guessing.
                  </p>
                </div>
                <Switch checked={useProspeo} onCheckedChange={setUseProspeo} />
              </div>
            ) : (
              <p className="text-xs text-muted-foreground">
                Prospeo is not connected. This run will use pattern guessing only.
              </p>
            )}
          </div>
          ...
          <DialogFooter className="pt-2">
            <Button
              variant="outline"
              onClick={() => setIsEmailEnrichOpen(false)}
              disabled={emailEnrichMutation.isPending}
            >
              Cancel
            </Button>
            <Button
