...
export function useTestFirecrawlConnection() {
  return useMutation({
    mutationFn: async (
      params:
        | string
        | { apiKey: string; provider?: "firecrawl" | "crawl4ai"; baseUrl?: string }
    ) => {
      const body =
        typeof params === "string"
          ? { apiKey: params, provider: "firecrawl" as const }
          : params;
      return await post<FirecrawlTestResult>(ENDPOINTS.RESEARCH.CONNECTION_TEST, body);
    },
  });
}
