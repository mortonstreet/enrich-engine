...
function FirecrawlConnectionSettings() {
  ...
  const crawl4aiConnection = vendorsData?.data?.find(v => v.provider === "crawl4ai");
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Firecrawl powers the hosted research path. For self-hosted OSS mode, connect
          the <span className="font-medium">Crawl4AI</span> vendor in Data Vendors.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {crawl4aiConnection?.isActive && (
          <div className="flex items-start gap-3 p-4 rounded-lg bg-green-50 border border-green-200 text-green-800">
            <Check className="h-5 w-5 flex-shrink-0 mt-0.5" />
            <div className="text-sm">
              <p className="font-medium">Crawl4AI connected</p>
              <p className="mt-1">
                Research tasks will prefer Crawl4AI + Qwen when available.
              </p>
            </div>
          </div>
        )}
