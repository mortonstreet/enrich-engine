...
export function DomainCsvFlow({ onBack, onSubmit, isSubmitting }: DomainCsvFlowProps) {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      if (!headers.includes("domain") && !headers.includes("website")) {
        setParseError("CSV must have a 'domain' or 'website' column");
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    await onSubmit({
      ...
      workflowType: ScrapeWorkflowType.DOMAIN_CSV,
      roleConfigs,
    });
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="border-b">
          <CardTitle>Domain CSV Upload</CardTitle>
          <CardDescription>
            Upload a CSV with company domains or websites. We'll find people matching your specified roles.
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-6 space-y-6">
          {/* Step 1: File Upload */}
          <div className="space-y-2">
            <label className="text-sm font-medium">1. Upload Domain List</label>
            ...
            {rowCount && !parseError && (
              <div className="flex items-center gap-2 text-sm text-green-600">
                ...
                Found {rowCount} domains
              </div>
            )}

            <p className="text-xs text-muted-foreground">
              CSV should have a column named 'domain' or 'website' (e.g., acme.com, company.io)
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g., SaaS Companies List"
          ...
          {isValid && rowCount && (
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground">{rowCount}</span> domains.
              </p>
