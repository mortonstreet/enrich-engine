# OmniDial - EnrichEngine Integration Specification
...
This document specifies how OmniDial should integrate with EnrichEngine's External API to pull and sync lead lists for dialing campaigns.
...
EnrichEngine uses simple API key authentication for third-party integrations. There is no OAuth flow, no token refresh, and no user authorization screens. Users generate a static API key in EnrichEngine and enter it into OmniDial.
...
2. User copies the key into OmniDial's settings
3. OmniDial stores the key and uses it for all API requests
...
4. Name it (e.g., "OmniDial Integration")
...
### 4.1 OmniDial Configuration

Add EnrichEngine integration settings to OmniDial.
...
CREATE TABLE enrichengine_synced_lists (
  ...
  omnidial_campaign_id UUID REFERENCES campaigns(id) ON DELETE SET NULL,
...
class EnrichEngineSyncService {
  ...
  async syncList(enrichEngineListId: string, omniDialCampaignId: string): Promise<SyncResult> {
    ...
    try {
      ...
      for (const lead of leads) {
        try {
          // Map EnrichEngine lead to OmniDial contact
          const contactData = this.mapLeadToContact(lead, omniDialCampaignId);

          // Upsert contact (update if exists, create if not)
          const existing = await db.contacts.findFirst({
            where: {
              campaignId: omniDialCampaignId,
              OR: [
                { email: lead.email },
                { phone: lead.phone },
                { externalId: lead.id },
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Connect to EnrichEngine">
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          Connect your EnrichEngine account to import enriched leads directly into OmniDial campaigns.
        </p>
...
Implement rate limiting on the OmniDial side:
...
When available, OmniDial should register webhooks to receive real-time updates:
...
4. Use that API key for OmniDial development
...
4. **Server-to-Server** - Ideal for backend integrations like OmniDial
